...
class TestChirpZTransform(unittest.TestCase,SParameterCompareHelper):
    def testCZTResampleSame(self):
        ...
        sf2=si.sp.ResampledSParameters(sf,si.sp.FrequencyList().SetEvenlySpaced(20.e9,200),speed='slow')
    ...
    def testCZTResampleHigherFreq(self):
        os.chdir(os.path.dirname(os.path.realpath(__file__)))
        sf=si.sp.File('TestDut.s4p')
        sf2=si.sp.ResampledSParameters(si.sp.ResampledSParameters(sf,si.sp.FrequencyList().SetEvenlySpaced(40.e9,400)),si.sp.FrequencyList().SetEvenlySpaced(20.e9,200))
        sf2.WriteToFile('TestDutCmp.s4p')
        os.remove('TestDutCmp.s4p')
        self.assertTrue(self.SParametersAreEqual(sf2,sf,0.001),self.id()+'result not same')
    def testCZTResampleHigherRes(self):
        os.chdir(os.path.dirname(os.path.realpath(__file__)))
        sf=si.sp.File('TestDut.s4p')
        sf2=si.sp.ResampledSParameters(si.sp.ResampledSParameters(sf,si.sp.FrequencyList().SetEvenlySpaced(20.e9,400)),si.sp.FrequencyList().SetEvenlySpaced(20.e9,200))
        sf2.WriteToFile('TestDutCmp.s4p')
        os.remove('TestDutCmp.s4p')
        self.assertTrue(self.SParametersAreEqual(sf2,sf,0.001),self.id()+'result not same')
