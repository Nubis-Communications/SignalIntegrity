...
class Archive(list):
    ...
    def CopyArchiveFilesToDestination(self,archiveDir,projectFile=None):
        ...
        try:
            ...
            # files that mangling will handle (only s-parameter files, and only when the
            # option is on) are left out of the common root so mangling still gets first
            # crack at them instead of the root silently growing to include them
            mangleEligibleFiles=set(filename for filename in self.srcList
                                    if self.archiveNonRelativeFiles and MangledFileName(filename) != '')
            self.common=Archive._CommonRoot(projectDir,[filename for filename in self.srcList
                                                        if filename not in mangleEligibleFiles])
